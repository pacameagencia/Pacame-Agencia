...
export function buildEmail(lead, demoUrl) {
  ...
  const subject = pick(slug, ':subject', SUBJECTS)(lead.name, lead.city);
  const preheader = pick(slug, ':preheader', PREHEADERS)(lead.name);
  ...
  return { subject, preheader, greeting, opening, hook, photoNote, closing, signoff, postscript, cityMention };
}
