    ...
    return value ? (
      <Check
        className={`w-5 h-5 mx-auto ${
          featured ? "text-olympus-gold" : "text-emerald-400/80"
        }`}
      />
    ) : (
      <Minus className="w-4 h-4 mx-auto text-pacame-white/20" />
    );
  }
  return (
    <span
      className={`text-sm font-body ${
        featured ? "text-pacame-white" : "text-pacame-white/75"
      }`}
    >
      {value}
    </span>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      <div className="mb-10 text-center">
        <p className="text-[12px] font-body font-medium text-olympus-gold/70 mb-3 uppercase tracking-[0.22em]">
          Comparativa completa
        </p>
        <h2 className="font-accent font-bold text-section text-pacame-white mb-3 text-balance">
          Elige con{" "}
          <span className="gradient-text-gold">total claridad.</span>
        </h2>
        <p className="text-pacame-white/55 font-body max-w-xl mx-auto">
      ...
      <div className="overflow-x-auto -mx-6 px-6 pb-2">
        <table className="w-full border-separate border-spacing-0 min-w-[720px]">
          {/* Sticky header */}
          <thead className="sticky top-0 z-20 bg-pacame-black">
            <tr>
              <th className="text-left py-5 px-4 font-heading font-semibold text-pacame-white/50 text-sm uppercase tracking-wider w-[26%] bg-pacame-black border-b border-olympus-gold/20">
                Features
              </th>
              {plans.map((plan) => (
                <th
                  key={plan.slug}
                  className={`text-center py-5 px-4 bg-pacame-black border-b border-olympus-gold/20 ${
                    plan.featured ? "relative" : ""
                  }`}
                  style={plan.featured ? { width: "19%" } : { width: "18%" }}
                >
                  {plan.featured && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-olympus-gold text-pacame-black text-[10px] font-heading font-bold px-2.5 py-0.5 rounded-full inline-flex items-center gap-1 whitespace-nowrap">
                  ...
                  <div
                    className={`font-heading font-bold text-lg ${
                      plan.featured ? "text-olympus-gold" : "text-pacame-white"
                    }`}
                  >
                    {plan.tier}
                  </div>
                  <div className="font-heading font-bold text-2xl text-pacame-white mt-1">
                    {plan.price}€
                    <span className="text-xs text-pacame-white/50 font-body font-normal ml-0.5">
                      /mes
                    </span>
                  </div>
                </th>
            ...
            {groups.map((group) => (
              <Fragment key={group.title}>
                ...
                <tr>
                  ...
                  >
                    <span className="text-[11px] font-body font-semibold text-olympus-gold uppercase tracking-[0.18em]">
                      {group.title}
                    </span>
                  </td>
                </tr>
                    ...
                    >
                      <td className="py-3.5 px-4 border-b border-white/[0.04]">
                        <div className="flex items-center gap-1.5">
                          <span className="text-sm font-body text-pacame-white/80">
                            {feat.label}
                          </span>
                          {feat.tooltip && (
                            <button
                              ...
                              className="relative text-pacame-white/30 hover:text-olympus-gold transition-colors"
                              aria-label={`Info: ${feat.label}`}
                            >
                              <Info className="w-3.5 h-3.5" />
                              {tooltipId === tid && (
                                <span className="absolute z-30 left-full ml-2 top-1/2 -translate-y-1/2 bg-dark-elevated border border-olympus-gold/20 rounded-lg px-3 py-2 text-xs text-pacame-white/80 font-body w-56 shadow-apple whitespace-normal text-left">
                                  {feat.tooltip}
                                </span>
                              )}
                            </button>
                      ...
                      {feat.values.map((val, idx) => (
                        <td
                          ...
                          className={`py-3.5 px-4 text-center border-b border-white/[0.04] ${
                            plans[idx]?.featured
                              ? "bg-olympus-gold/[0.04] border-l border-r border-olympus-gold/20"
                              : ""
                          }`}
                        >
              ...
              {plans.map((plan) => (
                <td
                  ...
                  className={`py-6 px-4 text-center ${
                    plan.featured
                      ? "bg-olympus-gold/[0.04] border-l border-r border-b border-olympus-gold/20 rounded-b-xl"
                      : ""
                  }`}
                >
                  <Link
                    ...
                    className={`inline-flex items-center justify-center gap-1.5 w-full py-2.5 px-3 rounded-xl text-xs font-heading font-semibold transition ${
                      plan.featured
                        ...
                        : "bg-white/[0.05] hover:bg-white/[0.1] border border-white/10 text-pacame-white"
                    }`}
                  >
                    Empezar
      ...
      <p className="text-center text-xs text-pacame-white/40 font-body mt-6">
