  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (!entry.isIntersecting) setRevealProgress(0);
      },
      { threshold: 0.3 }
    );
  ...
  return (
    ...
    >
      {shouldShowMobileReveal ? (
        <p className={cn('relative z-10 select-none', textClassName)}>
          {chars.map((char, index) => {
            const charProgress = (revealProgress * chars.length - index) / 1;
            const isRevealed = Math.max(0, Math.min(1, charProgress)) > 0.5;
            return (
              <span
                key={index}
                style={{ transition: 'all 0.3s ease-out' }}
                className={isRevealed ? '' : 'text-white/10'}
              >
                {char}
              </span>
            );
        ...
        <>
          <p className={cn('relative z-10 text-white/10 select-none', textClassName)}>
            {text}
          </p>
          <motion.div
