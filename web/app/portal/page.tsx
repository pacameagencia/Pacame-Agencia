      ...
      <div className="bg-pacame-black min-h-screen flex items-center justify-center">
        <div className="w-full max-w-md px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-pacame-white/40 font-body mt-2">
          ...
          {sent ? (
            ...
            <form onSubmit={handleRequestAccess} className="space-y-4">
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-xl bg-dark-card border border-white/[0.08] text-pacame-white font-body placeholder:text-pacame-white/30 focus:border-electric-violet/50 outline-none"
              />
          ...
          <div className="flex items-center gap-2 justify-center mt-6 text-xs text-pacame-white/20 font-body">
    ...
    return (
      <div className="bg-pacame-black min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md px-6">
          ...
          <p className="text-sm text-pacame-white/40 font-body mb-6">{errorMsg}</p>
      ...
      <header className="border-b border-white/[0.06] bg-dark-elevated/50 backdrop-blur-sm sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-pacame-white/30 font-body hidden sm:block">Portal de cliente</span>
          </div>
          <div className="text-right">
            <div className="text-sm font-heading font-medium text-pacame-white">{client.business_name || client.name}</div>
            <div className="text-xs text-pacame-white/40 font-body">Plan {client.plan || "Personalizado"}</div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5 text-center">
            ...
            <div className="text-xs text-pacame-white/40 font-body">Contenido total</div>
          </div>
          <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5 text-center">
            ...
            <div className="text-xs text-pacame-white/40 font-body">Publicados</div>
          </div>
          <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5 text-center">
            <Clock className="w-6 h-6 text-amber-400 mx-auto mb-2" />
            <div className="font-heading font-bold text-2xl text-amber-400">{content.stats.pending}</div>
            <div className="text-xs text-pacame-white/40 font-body">Pendientes</div>
          </div>
          <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5 text-center">
            ...
            <div className="text-xs text-pacame-white/40 font-body">Pagos</div>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-6">
          ...
          {content.items.length === 0 ? (
            <p className="text-sm text-pacame-white/30 font-body text-center py-6">Aun no hay contenido creado</p>
          ) : (
              ...
              {content.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center gap-4 p-3 rounded-xl bg-white/[0.03] border border-white/[0.04]">
                    <FileText className="w-4 h-4 text-pacame-white/30 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <span className="text-sm text-pacame-white font-body">{item.title}</span>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] text-pacame-white/30 font-body">{item.type} · {item.platform}</span>
            ...
            <div className="space-y-2">
              {calls.map((call) => (
                <div key={call.id} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.04]">
                  <Phone className="w-4 h-4 text-pacame-white/30 flex-shrink-0 mt-0.5" />
        ...
        {/* Footer */}
        <div className="text-center text-xs text-pacame-white/20 font-body py-4">
