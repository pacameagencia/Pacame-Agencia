...
export default function ReferralsPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div>
        <h1 className="font-heading font-bold text-2xl text-pacame-white">Referidos y Partners</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5">
          <Users className="w-5 h-5 text-electric-violet mb-2" />
          <div className="font-heading font-bold text-2xl text-pacame-white">{partners.length}</div>
          <div className="text-xs text-pacame-white/40 font-body">Colaboradores</div>
        </div>
        <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5">
          <UserPlus className="w-5 h-5 text-neon-cyan mb-2" />
          <div className="font-heading font-bold text-2xl text-neon-cyan">{referrals.length}</div>
          <div className="text-xs text-pacame-white/40 font-body">Referidos totales</div>
        </div>
        <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5">
          <CheckCircle2 className="w-5 h-5 text-lime-pulse mb-2" />
          <div className="font-heading font-bold text-2xl text-lime-pulse">{converted}</div>
          <div className="text-xs text-pacame-white/40 font-body">Convertidos</div>
        </div>
        <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-5">
          <DollarSign className="w-5 h-5 text-amber-signal mb-2" />
          <div className="font-heading font-bold text-2xl text-amber-signal">{totalEarned.toLocaleString("es-ES")}€</div>
          <div className="text-xs text-pacame-white/40 font-body">Comisiones pagadas</div>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-heading font-semibold text-lg text-pacame-white">Colaboradores</h2>
          <a
            href="/colabora"
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs text-electric-violet hover:underline font-body flex items-center gap-1"
          >
              ...
              <div key={p.id} className="flex items-center gap-4 p-4 rounded-xl bg-white/[0.03] border border-white/[0.04]">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-heading font-semibold text-sm text-pacame-white">{p.name}</span>
                    <span
                  ...
                  <div className="flex items-center gap-3 mt-1 text-xs text-pacame-white/40 font-body">
                    <span>{p.email}</span>
                    <span>·</span>
                    <span>{p.total_referrals} referidos</span>
                    <span>·</span>
                    <span className="text-lime-pulse">{p.total_conversions} convertidos</span>
                  </div>
                </div>

                <div className="flex items-center gap-3 flex-shrink-0">
                  <div className="text-right">
                    <div className="font-heading font-bold text-sm text-pacame-white">
                      {Number(p.total_earned).toLocaleString("es-ES")}€
                    </div>
                    <div className="text-[10px] text-pacame-white/30 font-body">ganado</div>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  >
                    {copiedCode === p.partner_code ? (
                      <><CheckCircle2 className="w-3 h-3 text-lime-pulse" />Copiado</>
                    ) : (
      ...
      {/* Recent referrals */}
      <div className="rounded-2xl bg-dark-card border border-white/[0.06] p-6">
        <h2 className="font-heading font-semibold text-lg text-pacame-white mb-5">Referidos recientes</h2>
        {referrals.length === 0 ? (
          <p className="text-sm text-pacame-white/30 font-body text-center py-6">Sin referidos aun</p>
        ) : (
