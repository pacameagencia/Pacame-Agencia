...
export default function ChatPage() {
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-8rem)]">
      {/* Sidebar */}
      <div className="w-64 flex-shrink-0 flex flex-col rounded-2xl bg-dark-card border border-white/[0.06] overflow-hidden">
        <div className="p-3 border-b border-white/[0.06]">
          <button
            onClick={newConversation}
            className="flex items-center gap-2 w-full px-3 py-2 rounded-lg bg-electric-violet/10 hover:bg-electric-violet/20 text-electric-violet text-sm font-body font-medium transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {loadingConversations ? (
            <div className="flex items-center justify-center py-8">
              <Loader2 className="w-4 h-4 animate-spin text-pacame-white/30" />
            </div>
          ) : conversations.length === 0 ? (
            <div className="text-center py-8">
              <MessageSquare className="w-6 h-6 text-pacame-white/15 mx-auto mb-2" />
              <p className="text-xs text-pacame-white/30 font-body">Sin conversaciones</p>
            </div>
          ) : (
              ...
              return (
                <button
                  ...
                  className={`group flex items-start gap-2 w-full px-3 py-2.5 rounded-lg text-left transition-all ${
                    isActive ? "bg-white/[0.08] border border-white/[0.1]" : "hover:bg-white/[0.04] border border-transparent"
                  }`}
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-pacame-white/80 font-body truncate">{title}</div>
                    <div className="text-[10px] text-pacame-white/30 font-body mt-0.5">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="font-heading font-bold text-2xl text-pacame-white">Chat con Agentes</h1>
        ...
        <div className="relative mb-4">
          <button
            ...
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-dark-card border border-white/[0.06] hover:border-white/10 transition-all w-full"
          >
            ...
            <div className="text-left flex-1">
              ...
              <div className="text-xs text-pacame-white/40 font-body">{selectedAgent.role}</div>
            </div>
            <ChevronDown className={`w-4 h-4 text-pacame-white/30 transition-transform ${showAgentPicker ? "rotate-180" : ""}`} />
          </button>

          {showAgentPicker && (
            <div className="absolute top-full left-0 right-0 mt-2 rounded-xl bg-dark-card border border-white/[0.08] shadow-2xl z-20 overflow-hidden">
              {agents.map((agent) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <span className="text-xs text-pacame-white/40 font-body ml-2">{agent.role}</span>
                    </div>
                  </button>
                );
              })}
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto rounded-2xl bg-dark-card border border-white/[0.06] p-4 space-y-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <h3 className="font-heading font-semibold text-pacame-white mb-1">{selectedAgent.name}</h3>
              <p className="text-sm text-pacame-white/40 font-body max-w-md">
              ...
              <div key={i} className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                ...
                <div
                  className={`max-w-[75%] rounded-2xl px-4 py-3 ${
                    isUser
                      ? "bg-electric-violet/20 border border-electric-violet/30"
                ...
                >
                  ...
                  <p className="text-sm text-pacame-white/80 font-body whitespace-pre-wrap leading-relaxed">{msg.content}</p>
                  <div className="text-[10px] text-pacame-white/20 font-body mt-1.5">
            ...
            <div className="flex gap-3 justify-start">
              ...
              <div className="rounded-2xl px-4 py-3 bg-white/[0.04] border border-white/[0.06]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-pacame-white/40 font-body">{selectedAgent.name} esta pensando...</span>
                </div>
              </div>
            </div>
          )}
        ...
        <form onSubmit={sendMessage} className="mt-4 flex gap-3">
          <div className="flex-1 relative">
            <textarea
              ...
              rows={1}
              className="w-full px-4 py-3 rounded-xl bg-dark-card border border-white/[0.08] text-sm text-pacame-white font-body placeholder:text-pacame-white/30 focus:border-electric-violet/50 outline-none resize-none"
            />
          </div>
          <Button
            type="submit"
