  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-heading font-bold text-ink">Cerebro</h1>
      ...
      {/* 1. Plan del dia */}
      <section className="bg-paper-soft border border-ink/[0.06] rounded-2xl p-5">
        <h2 className="text-sm font-heading font-semibold text-ink mb-3">
        ...
        {!plan ? (
          ...
          <div className="space-y-3">
            {plan.summary && (
              <p className="text-sm text-ink/70 font-body">{plan.summary}</p>
            )}
                ...
                return (
                  <div key={key} className="bg-ink/50 rounded-xl p-3 border border-white/[0.04]">
                    ...
                    <div className="flex items-baseline gap-1 mt-1">
                      <span className="text-lg font-heading font-bold text-ink">{actual}</span>
                      <span className="text-xs text-ink/40">/ {target}</span>
                    </div>
            ...
            {plan.focus_niches && plan.focus_niches.length > 0 && (
              <div className="space-y-2">
                <div className="text-[11px] uppercase tracking-wider text-ink/40 font-body">
                  Focus del dia
                </div>
                <div className="flex flex-wrap gap-2">
                  {plan.focus_niches.map((n, i) => (
                    <span key={`n-${i}`} className="text-xs px-2 py-1 rounded-full bg-accent-gold/10 text-accent-gold font-mono">
                      {n}
                    </span>
                  ))}
                  {plan.focus_channels?.map((c, i) => (
                    <span key={`c-${i}`} className="text-xs px-2 py-1 rounded-full bg-brand-primary/10 text-brand-primary font-mono">
                      {c}
                    </span>
                  ))}
                </div>
      ...
      <section>
        ...
        {!data || data.proposals_pending.length === 0 ? (
          <p className="text-sm text-ink/40 font-body">Sin propuestas pendientes.</p>
        ) : (
          <div className="grid gap-3">
            {data.proposals_pending.map((p) => (
              <div
                key={p.id}
                className="bg-paper-soft border border-ink/[0.06] rounded-2xl p-4 flex flex-col lg:flex-row lg:items-start gap-4"
              >
                <div className="flex-1 min-w-0 space-y-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    {p.agent_slug && (
                      <span className="text-[10px] uppercase tracking-wide px-2 py-0.5 rounded bg-white/[0.05] text-ink/60 font-body">
                        {p.agent_slug}
                      </span>
                    )}
                    {p.priority != null && (
                  ...
                  <p className="text-sm text-ink font-body">{p.hypothesis}</p>
                  {p.rationale && (
                    <p className="text-xs text-ink/50 font-body">{p.rationale}</p>
                  )}
                  {p.expected_impact && (
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-ink/40 font-body w-16">
                      confidence
                    </span>
                    ...
                    <span className="text-[10px] text-ink/50 font-body">
      ...
      <section>
        ...
        {!data || data.experiments_running.length === 0 ? (
          <p className="text-sm text-ink/40 font-body">Sin experimentos en curso.</p>
        ) : (
          <div className="bg-paper-soft border border-ink/[0.06] rounded-2xl overflow-hidden">
            <table className="w-full text-xs font-body">
              <thead className="bg-white/[0.03] text-ink/40 uppercase tracking-wide">
                <tr>
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {data.experiments_running.map((e) => (
                  <tr key={e.id} className="text-ink/70">
                    <td className="px-3 py-2">
                      <div className="font-medium text-ink">{e.name}</div>
                      {e.hypothesis && (
                        <div className="text-ink/40 text-[11px]">{e.hypothesis}</div>
                      )}
                    </td>
      ...
      <section>
        ...
        {!data || data.linkedin_queue_pending.length === 0 ? (
          <p className="text-sm text-ink/40 font-body">Sin mensajes pendientes.</p>
        ) : (
          <div className="bg-paper-soft border border-ink/[0.06] rounded-2xl overflow-hidden">
            <table className="w-full text-xs font-body">
              <thead className="bg-white/[0.03] text-ink/40 uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {data.linkedin_queue_pending.map((it) => (
                  <tr key={it.id} className="text-ink/70 align-top">
                    <td className="px-3 py-3">
                      <div className="font-medium text-ink">{it.target_name}</div>
                      <a
                        href={it.linkedin_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-brand-primary hover:underline text-[11px] inline-flex items-center gap-1"
                      >
                    ...
                    <td className="px-3 py-3">
                      <pre className="text-[11px] text-ink/70 whitespace-pre-wrap max-w-[500px] font-body">
                        {it.message_body}
                      </pre>
                    </td>
                    <td className="px-3 py-3">
                      <div className="flex flex-col gap-1.5 items-end">
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 rounded bg-white/[0.04] hover:bg-white/[0.08] text-ink/70 text-[11px] transition"
                        >
                        ...
                        <button
                          ...
                          disabled={actingOn === it.id}
                          className="flex items-center gap-1 px-2 py-1 rounded bg-white/[0.04] hover:bg-white/[0.08] text-ink/60 text-[11px] transition disabled:opacity-50"
                        >
      ...
      <section>
        ...
        {channelRows.length === 0 ? (
          <p className="text-sm text-ink/40 font-body">Sin datos.</p>
        ) : (
          <div className="bg-paper-soft border border-ink/[0.06] rounded-2xl overflow-hidden">
            <table className="w-full text-xs font-body">
              <thead className="bg-white/[0.03] text-ink/40 uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {channelRows.map((r) => (
                  <tr key={r.key} className="text-ink/70">
    ...
    <div
      className={`rounded-xl p-3 border ${
        accent
          ...
          : "bg-paper-soft border-ink/[0.06]"
      }`}
    >
      <div className="flex items-center gap-2 text-ink/50 text-[11px] uppercase tracking-wide font-body">
        {icon}
        {label}
      </div>
      <div className="text-xl font-heading font-bold text-ink mt-1">{value}</div>
    </div>
  );
}
