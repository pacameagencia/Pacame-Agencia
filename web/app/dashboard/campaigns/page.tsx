  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading font-bold text-2xl text-ink">Campanas</h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
          <Megaphone className="w-6 h-6 text-brand-primary mx-auto mb-2" />
          <div className="font-heading font-bold text-2xl text-ink">{active.length}</div>
          <div className="text-xs text-ink/40 font-body">Activas</div>
        </div>
        <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
          <DollarSign className="w-6 h-6 text-mint mx-auto mb-2" />
          <div className="font-heading font-bold text-2xl text-mint">{totalSpent.toLocaleString("es-ES")}€</div>
          <div className="text-xs text-ink/40 font-body">Gastado / {totalBudget.toLocaleString("es-ES")}€</div>
        </div>
        <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
          <Eye className="w-6 h-6 text-mint mx-auto mb-2" />
          <div className="font-heading font-bold text-2xl text-mint">{totalImpressions.toLocaleString("es-ES")}</div>
          <div className="text-xs text-ink/40 font-body">Impresiones</div>
        </div>
        <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
          <MousePointerClick className="w-6 h-6 text-brand-primary mx-auto mb-2" />
          <div className="font-heading font-bold text-2xl text-ink">{totalClicks.toLocaleString("es-ES")}</div>
          <div className="text-xs text-ink/40 font-body">Clicks</div>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-2xl bg-paper-deep border border-brand-primary/30 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-heading font-semibold text-ink">Nueva campana</h2>
      ...
      <div className="space-y-3">
        {!loading && campaigns.length === 0 && (
          <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-12 text-center">
            <Megaphone className="w-8 h-8 text-ink/20 mx-auto mb-3" />
            <p className="text-sm text-ink/40 font-body">Sin campanas</p>
            <p className="text-xs text-ink/50 font-body mt-1">Crea tu primera campana de ads</p>
          </div>
        )}
        {campaigns.map((c) => {
          ...
          return (
            <div key={c.id} className="rounded-2xl bg-paper-deep border border-ink/[0.06] hover:border-white/10 p-5 transition-all">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-heading font-semibold text-ink">{c.campaign_name}</h3>
                    <span
                  ...
                  <div className="flex items-center gap-4 flex-wrap text-xs text-ink/40 font-body">
                    ...
                    {c.budget_spent > 0 && (
                      <span className="text-brand-primary">Gastado: {Number(c.budget_spent).toLocaleString("es-ES")}€</span>
                    )}
                  </div>
                  {/* Performance metrics */}
                  {(perf.impressions || perf.clicks || perf.conversions) && (
                    <div className="flex items-center gap-4 mt-3 text-xs font-body">
                      ...
                      {perf.ctr && <span className="text-mint">CTR: {perf.ctr.toFixed(2)}%</span>}
                      {perf.cpc && <span className="text-ink/50">CPC: {perf.cpc.toFixed(2)}€</span>}
                      {perf.conversions && (
                        <span className="text-mint">
                          <TrendingUp className="w-3 h-3 inline mr-1" />{perf.conversions} conv
                        </span>
                      )}
                      {perf.roas && <span className="text-mint font-medium">ROAS: {perf.roas.toFixed(1)}x</span>}
                    </div>
                  )}
                </div>
