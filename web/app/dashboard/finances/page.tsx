  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading font-bold text-2xl text-ink">Finanzas</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
          ...
          <div className="text-xs text-ink/40 font-body">Ingresos este mes</div>
        </div>
        <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
          <TrendingDown className="w-6 h-6 text-red-400 mx-auto mb-2" />
          <div className="font-heading font-bold text-2xl text-red-400">{expenses.toLocaleString("es-ES")}€</div>
          <div className="text-xs text-ink/40 font-body">Gastos este mes</div>
        </div>
        <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
          ...
          <div className="text-xs text-ink/40 font-body">Beneficio neto</div>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-2xl bg-paper-deep border border-brand-primary/30 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-heading font-semibold text-ink">Nueva transaccion</h2>
      ...
      <div className="space-y-2">
        {!loading && transactions.length === 0 && (
          <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-12 text-center">
            <DollarSign className="w-8 h-8 text-ink/20 mx-auto mb-3" />
            <p className="text-sm text-ink/40 font-body">Sin transacciones</p>
          </div>
        )}
        {transactions.map((t) => {
          const isIncome = t.type === "income";
          return (
            <div key={t.id} className="flex items-center gap-4 p-4 rounded-xl bg-paper-deep border border-ink/[0.06]">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-heading font-medium text-ink">{t.description || t.category || (isIncome ? "Ingreso" : "Gasto")}</span>
                  {t.category && t.description && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/[0.05] text-ink/40 font-body">{t.category}</span>
                  )}
                </div>
                <div className="flex items-center gap-3 mt-0.5">
                  <span className="text-xs text-ink/30 font-body">{t.date}</span>
                  {t.invoice_number && <span className="text-xs text-ink/20 font-body">#{t.invoice_number}</span>}
                </div>
              </div>
