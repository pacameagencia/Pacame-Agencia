  ...
  return (
    <div className="bg-paper min-h-screen">
      ...
      <section className="relative pt-36 pb-20 overflow-hidden">
        <div className="absolute top-1/3 right-1/4 w-[500px] h-[300px] bg-mint/[0.05] rounded-full blur-[200px] pointer-events-none" />
        ...
        <ScrollReveal className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass text-xs font-body text-ink/60 mb-6">
            <Calculator className="w-3.5 h-3.5 text-mint" />
          ...
          <p className="text-lg text-ink/60 font-body font-light max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl glass p-6">
                <h2 className="font-heading font-semibold text-lg text-ink mb-5">Tu negocio</h2>

                <div className="space-y-5">
                  <div>
                    <label className="block text-xs text-ink/50 font-body mb-2">Sector</label>
                    <select
                      value={sector}
                      onChange={(e) => setSector(e.target.value)}
                      className="w-full h-11 px-4 rounded-xl bg-paper-deep border border-ink/[0.06] text-ink font-body text-sm focus:border-brand-primary outline-none"
                    >
                  ...
                  <div>
                    <label className="block text-xs text-ink/50 font-body mb-2">
                      Ticket medio por cliente: <strong className="text-ink">{ticketMedio}€</strong>
                    </label>
                    <input
                    ...
                    />
                    <div className="flex justify-between text-[10px] text-ink/50 font-body mt-1">
                  ...
                  <div>
                    <label className="block text-xs text-ink/50 font-body mb-2">
                      Clientes actuales al mes: <strong className="text-ink">{clientesMes}</strong>
                    </label>
                    <input
              ...
              <CardTiltContent>
              <div className="rounded-2xl glass p-6">
                <h2 className="font-heading font-semibold text-lg text-ink mb-5">Inversion</h2>
                    ...
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-xl border transition-all ${
                        selectedPack === i
                          ...
                          : "bg-white/[0.03] border-ink/[0.06] hover:border-white/10"
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="font-heading font-semibold text-sm text-ink">{opt.label}</span>
                          <p className="text-xs text-ink/60 font-body mt-0.5">{opt.description}</p>
                        </div>
                        <span className="font-heading font-bold text-lg text-ink">{opt.monthly}€<span className="text-xs text-ink/60">/mes</span></span>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {!calculated && (
                <MagneticButton>
                  ...
                  >
                    <button onClick={() => setCalculated(true)} className="flex items-center gap-2 text-ink w-full justify-center">
            ...
            <ScrollReveal className={`space-y-5 transition-opacity duration-500 ${calculated ? "opacity-100" : "opacity-30"}`}>
              ...
              <div className="rounded-3xl glass p-8 text-center card-golden-shine">
                <p className="text-xs text-ink/60 font-body uppercase tracking-widest mb-2">ROI estimado</p>
                <div className={`font-heading font-bold text-6xl mb-2 ${roi > 0 ? "text-mint" : "text-accent-gold"}`}>
                  {roi}%
                </div>
                <p className="text-sm text-ink/50 font-body">retorno anual sobre tu inversion</p>
              </div>
              </CardTiltContent>
              </CardTilt>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                  <Users className="w-5 h-5 text-mint mx-auto mb-2" />
                  <div className="font-heading font-bold text-2xl text-mint">{newLeads}</div>
                  <div className="text-[10px] text-ink/60 font-body">leads nuevos/mes</div>
                </div>
                <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                  <Target className="w-5 h-5 text-brand-primary mx-auto mb-2" />
                  <div className="font-heading font-bold text-2xl text-brand-primary">{newClients}</div>
                  <div className="text-[10px] text-ink/60 font-body">clientes nuevos/mes</div>
                </div>
                <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                  <DollarSign className="w-5 h-5 text-mint mx-auto mb-2" />
                  <div className="font-heading font-bold text-2xl text-mint">+{revenueIncrease}€</div>
                  <div className="text-[10px] text-ink/60 font-body">ingresos extra/mes</div>
                </div>
                <div className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                  <BarChart3 className="w-5 h-5 text-accent-gold mx-auto mb-2" />
                  <div className="font-heading font-bold text-2xl text-accent-gold">{paybackMonths > 12 ? "12+" : paybackMonths}</div>
                  <div className="text-[10px] text-ink/60 font-body">meses payback</div>
                </div>
              </div>

              <div className="rounded-2xl glass p-6">
                <h3 className="font-heading font-semibold text-ink mb-3">Resumen anual</h3>
                <div className="space-y-3 text-sm font-body">
                  <div className="flex justify-between">
                    <span className="text-ink/50">Inversion anual</span>
                    <span className="text-ink font-heading font-semibold">{yearlyCost.toLocaleString("es-ES")}€</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-ink/50">Ingresos extra estimados</span>
                    <span className="text-mint font-heading font-semibold">+{yearlyRevenue.toLocaleString("es-ES")}€</span>
                  </div>
                  <div className="border-t border-ink/[0.06] pt-3 flex justify-between">
                    <span className="text-ink/50">Beneficio neto estimado</span>
                    <span className="text-mint font-heading font-bold text-lg">+{(yearlyRevenue - yearlyCost).toLocaleString("es-ES")}€</span>
                  </div>
                </div>
              </div>
              ...
              <p className="text-[10px] text-ink/50 font-body text-center">
