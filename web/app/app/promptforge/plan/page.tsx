...
import { CheckoutCTA } from "../../asesor-pro/ajustes/plan/CheckoutCTA";

export const dynamic = "force-dynamic";

const TIER_NARRATIVE: Record<string, { headline: string; sub: string }> = {
  starter: { headline: "Empieza", sub: "50 prompts/mes, 2 variantes. Para probar." },
  pro: { headline: "Pro", sub: "500 prompts/mes, 3 variantes y vídeo." },
  studio: { headline: "Studio", sub: "Ilimitado, 5 variantes, API y vídeo." },
};
...
export default async function PromptForgePlanPage({
  searchParams,
}: {
  ...
  if (!product) {
    return <p className="font-sans text-ink-mute">Producto no encontrado.</p>;
  }
    ...
    <div className="space-y-8 max-w-5xl">
      ...
      {sp.checkout === "success" && (
        <div className="bg-green-600/10 border border-green-600/30 text-green-700 px-4 py-3 font-sans text-sm flex items-center gap-2">
          ...
          Pago procesado. Tu plan se actualizará en unos segundos.
        </div>
      )}
          ...
          return (
            <article
              ...
              className={`relative bg-paper border-2 ${
                tier.recommended ? "border-terracotta-500" : "border-ink/15"
              } p-6 flex flex-col`}
            >
              {tier.recommended && (
      ...
      <p className="font-mono text-[11px] uppercase tracking-[0.15em] text-ink-mute">
        Pagos procesados por Stripe. Cancela en cualquier momento.
      </p>
    </div>
  );
}
