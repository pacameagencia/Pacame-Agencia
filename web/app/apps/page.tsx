    ...
    <div className="bg-pacame-black min-h-screen">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-xl text-pacame-white/60 font-body max-w-2xl mx-auto mb-10 font-light">
            Instala, configura en 10 minutos, deja que la IA haga el trabajo pesado. Sin
            permanencia, cancela cuando quieras.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-pacame-white/60 font-body">
      ...
      <section className="py-16">
        <div className="max-w-6xl mx-auto px-6">
          {apps.length === 0 ? (
            <div className="text-center text-pacame-white/50 font-body py-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {apps.map((app) => (
                <Link
                  key={app.slug}
                  href={`/apps/${app.slug}`}
                  className="group relative rounded-2xl p-7 bg-dark-card border border-white/[0.06] hover:border-olympus-gold/40 transition card-golden-shine"
                >
                  {app.is_featured && (
                    <div className="absolute top-5 right-5 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider text-olympus-gold bg-olympus-gold/10 border border-olympus-gold/20 rounded-full px-2 py-0.5">
                      <Sparkles className="w-2.5 h-2.5" />
                      Destacado
                    </div>
                  )}

                  <div className="text-[11px] font-mono uppercase tracking-wider text-pacame-white/40 mb-3">
                    {app.category || "app"}
                  </div>

                  <h2 className="font-heading font-bold text-2xl text-pacame-white mb-2 group-hover:text-olympus-gold transition">
                    {app.name}
                  </h2>
                  <p className="text-pacame-white/70 font-body mb-5">
                    {app.tagline}
                  </p>
                  ...
                  {app.benefits && app.benefits.length > 0 && (
                    <ul className="space-y-2 mb-6">
                      {app.benefits.slice(0, 3).map((b, i) => (
                        <li
                          key={i}
                          className="flex items-start gap-2 text-sm text-pacame-white/70 font-body"
                        >
      ...
      <section className="py-20">
        <div className="max-w-2xl mx-auto px-6 text-center">
          <h2 className="font-accent font-bold text-2xl sm:text-3xl text-pacame-white mb-4">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 bg-olympus-gold text-pacame-black font-heading font-semibold px-6 py-3 rounded-xl hover:bg-olympus-gold/90 transition"
          >
            Hablar con Pablo
            <ArrowRight className="w-4 h-4" />
          </a>
