    ...
    return (
      <div className="bg-paper min-h-screen flex items-center justify-center">
        <div className="text-center">
          <Award className="w-12 h-12 text-ink/20 mx-auto mb-4" />
          <h1 className="font-heading font-bold text-xl text-ink mb-2">Codigo no encontrado</h1>
          <p className="text-sm text-ink/40 font-body mb-6">Este codigo de partner no existe o ha sido desactivado.</p>
    ...
    <div className="bg-paper min-h-screen">
      ...
      <section className="relative pt-24 pb-12 overflow-hidden">
        ...
        <ScrollReveal className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="font-body text-sm text-ink/40 mb-1">Panel de partner</p>
          <h1 className="font-heading font-bold text-3xl text-ink mb-2">Hola, {data.name}</h1>
          <span
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 pb-20 space-y-6">
        ...
        <StaggerContainer className="grid grid-cols-2 md:grid-cols-4 gap-4" staggerDelay={0.08}>
          <StaggerItem>
            <CardTilt tiltMaxAngle={8}>
              <CardTiltContent className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                <Users className="w-5 h-5 text-mint mx-auto mb-2" />
                <div className="font-heading font-bold text-2xl text-mint">{data.total_referrals}</div>
                <div className="text-xs text-ink/40 font-body">Referidos</div>
              </CardTiltContent>
            </CardTilt>
          </StaggerItem>
          <StaggerItem>
            <CardTilt tiltMaxAngle={8}>
              <CardTiltContent className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                ...
                <div className="font-heading font-bold text-2xl text-mint">{data.total_conversions}</div>
          ...
          <StaggerItem>
            <CardTilt tiltMaxAngle={8}>
              <CardTiltContent className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                ...
                <div className="font-heading font-bold text-2xl text-accent-gold">{Number(data.total_earned).toLocaleString("es-ES")}€</div>
                <div className="text-xs text-ink/40 font-body">Total ganado</div>
              </CardTiltContent>
            </CardTilt>
          </StaggerItem>
          <StaggerItem>
            <CardTilt tiltMaxAngle={8}>
              <CardTiltContent className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-5 text-center">
                <TrendingUp className="w-5 h-5 text-brand-primary mx-auto mb-2" />
                <div className="font-heading font-bold text-2xl text-brand-primary">{pendingCommissions.toLocaleString("es-ES")}€</div>
                <div className="text-xs text-ink/40 font-body">Pendiente cobro</div>
              </CardTiltContent>
            </CardTilt>
          </StaggerItem>
        </StaggerContainer>
        ...
        <ScrollReveal delay={0.1} className="rounded-2xl glass p-6">
          <h2 className="font-heading font-semibold text-lg text-ink mb-4">Tu enlace de referido</h2>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 flex items-center gap-3 px-4 py-3 rounded-xl bg-paper-deep border border-ink/[0.08]">
              <Share2 className="w-4 h-4 text-ink/30 flex-shrink-0" />
              <span className="text-sm text-ink font-mono truncate">{referralLink}</span>
            </div>
            <Button
          ...
          <p className="text-xs text-ink/30 font-body mt-3">
            Comparte este enlace. Cuando alguien contrate, tu ganas un <strong className="text-ink/50">{data.commission_first_pct}% del primer pago</strong> + <strong className="text-ink/50">{data.commission_recurring_pct}% recurrente</strong>.
          </p>
        </ScrollReveal>

        {/* Tier progress */}
        {data.next_tier && (
          <ScrollReveal delay={0.15} className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-6">
            <h2 className="font-heading font-semibold text-ink mb-3">Progreso al siguiente tier</h2>
        ...
        {/* Recent referrals */}
        <ScrollReveal delay={0.1} className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-6">
          <h2 className="font-heading font-semibold text-ink mb-4">Tus referidos</h2>
          {(data.referrals || []).length === 0 ? (
            <div className="text-center py-8">
              <Users className="w-8 h-8 text-ink/15 mx-auto mb-3" />
              <p className="text-sm text-ink/30 font-body">Aun sin referidos. Comparte tu enlace para empezar a ganar.</p>
            </div>
          ) : (
        ...
        {(data.commissions || []).length > 0 && (
          <ScrollReveal delay={0.15} className="rounded-2xl bg-paper-deep border border-ink/[0.06] p-6">
            <h2 className="font-heading font-semibold text-ink mb-4">Historial de comisiones</h2>
            <div className="space-y-2">
              {data.commissions.map((c) => (
                <div key={c.id} className="flex items-center justify-between p-3 rounded-lg bg-white/[0.02]">
                  <div className="flex items-center gap-3">
                    <DollarSign className="w-3.5 h-3.5 text-mint" />
                    <span className="text-sm font-heading font-semibold text-ink">{Number(c.amount).toLocaleString("es-ES")}€</span>
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-body ${
                        c.status === "paid" ? "bg-mint/20 text-mint" : "bg-accent-gold/20 text-accent-gold"
                      }`}
                    >
        ...
        <ScrollReveal className="text-center pt-4">
          <p className="text-xs text-ink/50 font-body mb-4">
            ...
            <a href="mailto:hola@pacameagencia.com" className="text-brand-primary/60 hover:underline">hola@pacameagencia.com</a>
          </p>
